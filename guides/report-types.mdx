...
| **Parameter** | **Description** | **Allowed values** | **Default** |
| ------------- | --------------- | ------------------ | ----------- |
| `inject_output` | Return raw C2PA tool output as-is, without Probe.dev metadata wrapper (enabled by default for single-tool convenience) | `true`, `false` | `true` |
...
<Tip>
**Why `inject_output=true` by default?** C2PA analysis returns a single, self-contained report making it ideal for raw output. This gives you the same experience as running the tool locally. For usage statistics or metadata, set `inject_output=false`.
</Tip>
