...
### Using a pre-signed Amazon S3  URL
...
The example query string would grant read‑only access to `sample-video.mp4` for 24 hours (86 400 seconds).

#### **How to generate a pre‑signed S3 URL**
...
>
> The PHP example below shows a one‑line swap: replace the local shell invocation with an HTTPS GET to the Probe API while keeping every `ffprobe`_parameter intact._
>
> ```php
